...
function buildDesignData(markdownText, markdownFilePath, htmlFilePath) {
  ...
  const visualTheme = findSection(sections, "Visual Theme") ?? findSection(sections, "Style In One Sentence");
  const designDna = findSection(sections, "Design DNA");
  const colorPalette = findSection(sections, "Color Palette") ?? findSection(sections, "Color Logic");
  const typography = findSection(sections, "Typography");
  const components = findSection(sections, "Component");
  const layout = findSection(sections, "Layout") ?? findSection(sections, "Composition Grammar");
  const depth = findSection(sections, "Depth") ?? designDna;
  const dosAndDonts = findSection(sections, "Do");
  const responsive = findSection(sections, "Responsive");
  const promptGuide = findSection(sections, "Agent Prompt Guide") ?? findSection(sections, "Transfer Prompt");
  const observedPages = findSection(sections, "Observed Pages") ?? findSection(sections, "Source & Confidence");
  ...
  const dnaRows = parseFirstTable(designDna?.content ?? "");
  const keyCharacteristics = parseBulletItems(findSubsectionContent(visualTheme, "Key Characteristics"));
  if (!keyCharacteristics.length) {
    keyCharacteristics.push(...dnaRows.slice(0, 6).map((row) => cleanText(row.transferable_rule ?? row.observation)));
  }

  const colorRows = parseFirstTable(colorPalette?.content ?? "").map((row) => ({
    role: cleanText(row.role),
    name: cleanText(row.semantic_name ?? row.semantic ?? row.name ?? row.role),
    rawValue: cleanText(row.value ?? row.measured_value),
    swatch: extractColor(cleanText(row.value ?? row.measured_value)),
    usage: cleanText(row.usage ?? row.usage_rule),
  }));
  ...
  const typographyRows = parseFirstTable(findSubsectionContent(typography, "Hierarchy") || typography?.content || "").map(
    (row) => {
      const size = cleanText(row.size ?? row.desktop);
      const combinedMetrics = cleanText(row.weight_line_height);
      return {
        role: cleanText(row.role),
        font: cleanText(row.font ?? row.family),
        size,
        weight: cleanText(row.weight ?? combinedMetrics.split("/")[0]),
        lineHeight: cleanText(row.line_height ?? combinedLineHeight(combinedMetrics, size)),
        tracking: cleanText(row.letter_spacing),
        notes: cleanText(row.notes ?? row.rule),
      };
    }
  );
  ...
  const depthRows = parseFirstTable(depth?.content ?? "").map((row) => ({
    level: cleanText(row.level ?? row.dimension),
    treatment: cleanText(row.treatment ?? row.observation),
    use: cleanText(row.use ?? row.transferable_rule),
  }));

  const breakpointRows = parseFirstTable(findSubsectionContent(responsive, "Breakpoints") || responsive?.content || "").map(
    (row) => ({
      name: cleanText(row.name ?? row.condition),
      width: cleanText(row.width ?? row.what_stays_invariant),
      change: cleanText(row.key_changes ?? row.key_change ?? row.changes ?? row.what_transforms),
    })
  );
  ...
  const readyPrompt = getLeadParagraphs(findSubsectionContent(promptGuide, "Ready-to-Use Prompt"))[0]
    ?? getLeadParagraphs(promptGuide?.content ?? "")[0]
    ?? "";
...
function renderComponentCard(buttonSpec, cardDefinitions, theme, typographySpec, spacingSpec) {
  const cardSummary = cardDefinitions.get("Surface style") || "Measured surface, border, shadow, and spacing treatment.";

  return `<div class="component-card">
    <div class="button-row">
      <span class="cta cta-primary">${escapeHtml(buttonSpec.primaryLabel)}</span>
      <span class="cta cta-secondary">${escapeHtml(buttonSpec.secondaryLabel)}</span>
    </div>
...
function extractDocumentTitle(text) {
  const match = text.match(/^#\s+(?:Design System|STYLE):\s+(.+)$/m);
...
function buildButtonSpec(primaryButton, secondaryButton, typographySpec, spacingSpec) {
  ...
  return {
    primaryLabel: cleanText(primaryButton.get("Label") ?? primaryButton.get("Text") ?? "Primary action"),
    secondaryLabel: cleanText(secondaryButton.get("Label") ?? secondaryButton.get("Text") ?? "Secondary action"),
    primaryBackground,
    primaryText,
    secondaryBackground,
    secondaryText,
...
function buildThemeTokens(colorRows, quickColors, surfaceDefinitions, shadowDefinitions, cardDefinitions, typographySpec) {
  const rowText = (row) => `${row.role} ${row.name} ${row.usage}`;
  const fromRows = (patterns, fallback, reject = []) =>
    colorRows.find((row) => {
      const text = rowText(row);
      return patterns.some((pattern) => pattern.test(text)) && !reject.some((pattern) => pattern.test(text));
    })?.swatch ?? fallback;
  const fromQuick = (name) => extractColor(quickColors.get(name) ?? "");
  const fromQuickContains = (patterns) => {
    for (const [key, value] of quickColors.entries()) {
  ...
  const background =
    ...
    fromQuickContains([/background/i, /main background/i]) ??
    extractColor(surfaceDefinitions.get("Main page surface") ?? "") ??
    fromRows([/page background/i, /\bbackground\b/i, /page surface/i, /cloud/i], "#fafafa");
  const surface =
    fromQuick("Elevated surface") ??
    fromQuickContains([/surface/i, /raised/i]) ??
    extractColor(surfaceDefinitions.get("Elevated card and control surface") ?? "") ??
    fromRows([/elevated surface/i, /\bsurface\b/i, /raised/i, /white/i], "#ffffff", [/text/i]);
  const ink =
    fromQuick("Heading text") ??
    fromQuickContains([/primary text/i, /heading text/i]) ??
    fromRows([/primary text/i, /heading/i, /\bink\b/i, /\btext\b/i], "#171717", [/surface/i, /background/i, /border/i]);
  const muted =
    fromQuick("Body text") ??
    fromQuickContains([/secondary text/i, /tertiary text/i, /muted text/i, /body text/i]) ??
    fromRows([/secondary text/i, /tertiary text/i, /muted text/i, /body text/i, /supporting text/i, /quiet body/i, /slate/i], "#4d4d4d", [/surface/i, /background/i, /border/i]);
  const border =
    fromQuick("Ring / border") ??
    fromQuickContains([/border/i, /divider/i, /ring/i]) ??
    fromRows([/\bborder\b/i, /\bring\b/i, /\bdivider\b/i, /\bline\b/i], "#ebebeb", [/linear/i]);
  const accent =
    fromQuick("Accent / focus") ??
    fromQuickContains([/accent/i, /primary action/i, /focus/i]) ??
    fromRows([/\baccent\b/i, /\bfocus\b/i, /\bblue\b/i, /primary action/i], "#0072f5");
  const shadow =
...
function combinedLineHeight(value, fontSize) {
  const rawLineHeight = `${value ?? ""}`.split("/")[1]?.trim();
  if (!rawLineHeight) return "";
  if (/[a-z%]/i.test(rawLineHeight)) return rawLineHeight;
  const ratio = Number.parseFloat(rawLineHeight);
  const size = extractFirstNumber(fontSize);
  return Number.isFinite(ratio) && size != null ? `${Number((ratio * size).toFixed(3))}px` : rawLineHeight;
}
