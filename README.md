...
只需要提供一个网站 URL，HTML2Style 就会自动分析它的颜色、字体、间距、布局、图片、图标和响应式规则，生成一个可以直接交给 Agent 使用的**设计风格 Skill**，并附带 HTML 预览。
...
3. **复用网页风格：** 把生成的 Skill 交给任意 Agent，让它按照这套风格设计新的页面。
...
人只需要双击 `assets/preview.html`；Agent 只需要读取 `SKILL.md`。详细规则和数据会由 Skill 按需读取，不再让用户面对一堆入口文件。

下面是生成后的 HTML 预览示例。它同时展示设计规则和支撑规则的来源，而不只是一张参考截图。

<p align="center">
  <img src="docs/assets/style-board.png" alt="HTML2Style 生成的可视化风格板" width="920">
</p>

查看仓库中的[完整示例 Skill](examples/product-editorial/product-editorial-style/)。克隆仓库后，可以在本地双击其中的 `assets/preview.html`；示例使用虚构内容，不重新分发真实网站的品牌和素材。
...
只有需要保存原始证据、manifest 或自动化入口时，才使用旧的 `html2style bundle` 完整归档模式。

## 跨 session 复用

把整个设计风格 Skill 移动到新项目或 Agent 的 skills 目录，然后说：

```text
使用 $product-editorial-style，把这套网页风格应用到我的新任务。
```

不支持自动发现 Skill 的 Agent，也可以直接读取文件夹里的 `SKILL.md`。它不需要原聊天记录，也不需要再次打开参考网站。
...
- 中文用户默认生成 `zh-CN` 说明文件。
- 英文用户使用 `--locale en`。
- 生成 Skill 的 `SKILL.md` 和 `agents/openai.yaml` 会跟随语言生成。
